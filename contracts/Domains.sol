...
contract Domains is Ownable, ERC721URIStorage {
    ...
    // This could be extended to store addresses by chain or even other data
    // like profile picture, email, ...
    struct Record {
        address owner;
    }
    ...
    constructor(string memory _tld)
        payable
        ERC721("Alpaca Name Service", "ANS")
    {
        tld = _tld;
    ...
    function price(string calldata name) public pure returns (uint256) {
        ...
        require(len > 0, "Domain must have more than 0 characters");
        if (len == 3) {
            ...
            bytes(
                string(
                    abi.encodePacked(
                        '{"name": "',
                        _name,
                        '", "description": "A domain on the Alpaca name service", "image": "data:image/svg+xml;base64,',
